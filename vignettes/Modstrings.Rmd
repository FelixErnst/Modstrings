...
package: Modstrings
abstract: >
  Classes for RNA and DNA sequences containing modified nucleotides
output:
...
[@Boccaletto.2018], modified nucleotides are given. However in the Bioconductor
universe these information were not accessible, since they rely extensively on
special characters in the RNA modification alphabet.
...
normal `XString` classes within the bioconductor universe. Most of the 
...
In addition, one can also use the `alphabet()` function and subset to the desired
modifications.
...
qgr <- separate(qmrs)
qgr
combineIntoModstrings(mrs,qgr)
```

# Saving and reading ModString objects to file

The nucleotide sequences with modifications can be be saved to a `fasta` or
...
In principle post-translational modifications of proteins can also be 
implemented. However, a one letter alphabet of post-translational modifications
must be developed first. (Or does this already exist? Let us know.)

# Import example

This is a quick example shwoing how sequence information containing modified
